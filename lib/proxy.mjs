        ...
        while (a < budget) {
          ...
          try {
            const result = await attemptChannel({
              ...
              limiter: manager.limiter, agentId, routing, timing, deadlineAt,
            });
 ...
 */
async function rotate429Upstream({ plans, payload, signal, channel, sendFetch, requestId, takeKey }) {
  ...
  for (let i = 0; i < keyCount; i += 1) {
    ...
    if (upstream.ok) {
      ...
      log.info(
        `叠 Key 轮转：从 key#${plans[lastIndex] ? lastIndex + 1 : 1} 起共尝试 ${attempts}/${keyCount} 个 key，key#${slot.index + 1} 成功`,
        requestId,
      );
    ...
    if (action === 'next-key' && !isLast) {
      cancelUpstreamBody(upstream);                 // 429 等：不等待、不读体，立刻取下一个 Key
      stackedKeyStats.stackedKeyFastFailovers += 1;
      continue;
    }
    // 不可切 Key 的错误（如普通 400），或已扫完整圈：把响应交回渠道级错误处理
    settle();
    if (action === 'next-key' || isLast) stackedKeyStats.stackedKeyAllFailed += 1;
    log.warn(
      `叠 Key 轮转：共尝试 ${attempts}/${keyCount} 个 key，最终 HTTP ${upstream.status}，交回渠道级处理`,
      requestId,
    );
    return { upstream, index: slot.index, attempts };
  }

  // 一圈内没有任何可用的 HTTP 响应（全是网络错误）：交回最先/最后的网络错误分类
  settle();
  stackedKeyStats.stackedKeyAllFailed += 1;
...
async function attemptChannelInner({ channel, body, model, stream, res, requestId, clientProtocol = 'openai', agentId = null, routing = null, timing = null, deadlineAt = Infinity }) {
