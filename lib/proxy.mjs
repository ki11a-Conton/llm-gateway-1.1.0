...
async function attemptChannelInner({ channel, body, model, stream, res, requestId, clientProtocol = 'openai', agentId = null, routing = null, timing = null, deadlineAt = Infinity }) {
  const { url, headers, payload: basePayload } = channel.adapter.buildRequest({ channel, model, body, stream });
  ...
  if (!stream) {
    ...
    try {
      ...
      setGatewayHeaders(res, channel, model, agentId);
...
function setGatewayHeaders(res, channel, model, agentId = null) {
  try {
