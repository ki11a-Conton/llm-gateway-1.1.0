...
async function attemptChannelInner({ channel, body, model, stream, res, requestId, clientProtocol = 'openai', agentId = null, routing = null, timing = null, deadlineAt = Infinity }) {
  ...
  // 竞速胜出的 key 序号：多 key 渠道才回传响应头（单 key 时为 null，保持旧行为零变化）
  const keyInfo = plans.length > 1 ? { count: plans.length, index: null } : null;
  ...
  try {
    ...
    if (plans.length === 1) {
      ...
      // 叠加 key：并行竞速，第一个成功的 key 胜出（其余请求立刻取消）
...
function setGatewayHeaders(res, channel, model, agentId = null, keyInfo = null) {
  try {
    ...
    // 叠加 key 渠道：回传本次竞速胜出的 key 序号（只给序号，不回传 key 本身）
