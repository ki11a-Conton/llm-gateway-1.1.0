...
export class Channel {
  constructor(raw, routing, presets) {
    ...
    // 叠加多个 key。每次请求对这些 key **并行竞速**，第一个成功（2xx）的响应被采用，
    // 其余请求立刻取消（见 lib/proxy.mjs raceUpstream）。
   ...
   * 调用方（lib/proxy.mjs）据此对多个 key 并行竞速，第一个成功者胜出。
...
export class ChannelManager {
  ...
  addChannel(input = {}) {
    ...
    log.ok(
      `新增渠道 ${name}${probe.keyMissing ? '（未填 key，已停用）' : probe.usableKeys.length > 1 ? `（叠加 ${probe.usableKeys.length} 个 key，并行竞速）` : ''}`,
    );
