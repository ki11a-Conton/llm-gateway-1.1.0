...
export class Channel {
  constructor(raw, routing, presets) {
    ...
    // 多 key 渠道：apiKeys 数组（也兼容把 apiKey 直接写成数组）= 固定同一个 baseUrl，
    // 叠加多个 key。同一 baseUrl 下用哪种内部调度由 stackedKeyStrategy 决定：
    //   race       = 对所有 key **并行竞速**，第一个成功（2xx）的响应被采用，其余立刻取消
    //   rotate-429 = 环形游标逐个消费 key，429/可切 Key 错误立即换下一个（见 lib/proxy.mjs rotate429Upstream）
    // NOTE: 这属于"渠道路由选中某条渠道之后"的第二层调度（Stacked-Key Scheduling），
    //       绝不参与第一层 Channel Routing（routing.strategy / 两段式选路 / 渠道熔断）。
    // apiKey 仍保留"第一个 key"的旧语义：WorkBuddy token 续期 / 面板掩码 / 单 key 渠道全不受影响。
    const rawKeys = cfg.apiKeys !== undefined ? cfg.apiKeys : cfg.apiKey;
    const keyList = Array.isArray(rawKeys)
      ? rawKeys.map((k) => String(k ?? '').trim())
      : (rawKeys === undefined || rawKeys === null ? [] : [String(rawKeys).trim()]);
    this.apiKeys = keyList.filter((k) => k !== '');
    this.apiKey = this.apiKeys[0] ?? '';
    // 真正可用于请求的 key：过滤掉未展开的 ${ENV} 占位（环境变量没注入时不能拿它去撞上游）
    this.usableKeys = this.apiKeys.filter((k) => !/^\$\{[^}]*\}$/.test(k));
    // 叠 Key **内部**调度策略（只作用于"一条已选中渠道内部的多个 apiKeys"，与 routing.strategy 无关）：
    //   race       = 全量并发竞速（旧行为，默认）：所有 Key 同时发，第一个 2xx 胜出
    //   rotate-429 = 单发环形轮转：429/可切 Key 错误立即换下一个 Key；成功后下一条请求从后一个 Key 开始
    // 仅在 apiKeys.length > 1 时有意义；单 Key 渠道一律按旧单 Key 路径处理（归一为 race）。
    // NOTE: 这是"叠 Key 内部调度器"的配置，绝不参与渠道选路（两层边界见 lib/proxy.mjs 顶部注释）。
    const stackedRaw = cfg.stackedKeyStrategy;
    if (stackedRaw !== undefined && stackedRaw !== null && String(stackedRaw).trim() !== '') {
      const v = String(stackedRaw).trim().toLowerCase();
      if (v !== 'race' && v !== 'rotate-429') {
        throw new Error(`渠道 ${cfg.name}: 非法的 stackedKeyStrategy "${stackedRaw}"（只支持 race | rotate-429）`);
      }
      this.stackedKeyStrategy = this.usableKeys.length > 1 ? v : 'race';
    } else {
      this.stackedKeyStrategy = 'race';
    }
    ...
    const keyReady = this.usableKeys.length > 0;
    ...
    this.failed = 0;
    this.rr = 0;
    // 叠 Key 内部环形游标（仅 rotate-429 使用）：只属于本条渠道，热重载时同名渠道尽量继承。
    // NOTE: 这是 Key 级状态，与渠道级 round-robin 的渠道游标彻底分层，互不参与对方的调度。
    this.nextKeyIndex = 0;
  ...
  /**
   * 本条渠道本次请求要用的 key 列表（叠加 key 渠道 = 多个，普通渠道 = 1 个）。
   * race 模式下调用方（lib/proxy.mjs）据此对多个 key 并行竞速，第一个成功者胜出；
   * rotate-429 模式下只作为环形游标的候选池（见 takeNextRequestKey）。
   * 注意：这里**不做 key 级健康状态**——每次请求都把请求发给全部 key，
   * 由"谁先成功"自然决定结果，不记忆哪个 key 坏过。
   */
  get requestKeys() {
    return this.usableKeys.length ? this.usableKeys : (this.apiKey ? [this.apiKey] : []);
  }

  /**
   * 叠 Key 内部环形游标（rotate-429）：取出"本次请求要用的 key"并**立刻**把游标推进一格。
   *
   * 关键约束（TASK 2 / 5）：
   *   - 同步执行、函数内部绝不 await —— Node 单线程事件循环下，"读游标 + 写游标"之间没有
   *     await 间隙，多个并发请求不会读到同一个旧值（并发首发 Key 自然摊开，见 TASK 8）。
   *   - 游标推进只发生在这里；成功后**绝不**再自增一次（否则双推进，规则 D 被打乱）。
   *   - 只维护在内存（不写回 config.json）；每条渠道独享，不影响其它渠道、更不影响渠道路由。
   *
   * NOTE: 这是"一条已被选中的渠道内部"的 Key 轮转，绝不参与渠道选择。
   * @returns {{key: string, index: number, count: number}|null}
   */
  takeNextRequestKey() {
    const keys = this.requestKeys;
    if (!keys.length) return null;
    const index = ((this.nextKeyIndex % keys.length) + keys.length) % keys.length;
    // 规则 A：只要这个 key 真被发出去，游标就已前移——读取与推进一次完成。
    this.nextKeyIndex = (index + 1) % keys.length;
    return { key: keys[index], index, count: keys.length };
  }

  /**
   * 把叠 Key 环形游标**重置到第一个 Key**（rotate-429 的"整圈失败后回 K1 重扫"专用）。
   *
   * 为什么需要显式重置：takeNextRequestKey 是纯环形推进，跑完一整圈后游标会停在这一圈的
   * **起点**——若起点是 K3，下一圈仍从 K3 起。而"重扫一圈"的语义要求每次都从**第一个 Key**
   * 开始，所以由调用方在开新一圈前重置一次。
   *
   * NOTE: 同样只影响"一条已选中渠道内部的 Key 顺序"，不参与渠道选路。
   */
  resetKeyCursor() {
    this.nextKeyIndex = 0;
  }
  ...
  toJSON() {
    return {
      ...
      keyCount: this.usableKeys.length,
      // 叠 Key 内部策略：只在多 Key 渠道有意义（单 Key 返回 null，面板不显示无意义标签）
      stackedKeyStrategy: this.usableKeys.length > 1 ? this.stackedKeyStrategy : null,
      ...
      proxy: this.proxy,
      // 面板"编辑"回填用：WorkBuddy 账号 ID（非密钥，可回显）。
      // apiKey / refreshToken 一律只回长度，绝不回明文——编辑时留空即"保持不变"。
      userId: this.headers?.['X-User-Id'] || this.headers?.['x-user-id'] || '',
...
export class ChannelManager {
  ...
  load() {
    ...
    const channels = list.map((raw2) => {
      ...
      if (old && old.baseUrl === ch.baseUrl && old.protocol === ch.protocol) {
        ...
        // 叠 Key 环形游标：key 列表没变时继承（否则热重载会把轮转位置打回 K1，
        // 与"下一条请求从后一个 Key 开始"的规则冲突）。key 列表变了就归零重建。
        if (old.apiKeys.length === ch.apiKeys.length
          && old.apiKeys.every((k, i) => k === ch.apiKeys[i])) {
          ch.nextKeyIndex = old.nextKeyIndex;
        }
  ...
  /**
   * 拉取「上游模型列表」——**只读**操作：不改任何运行时状态，也不写 config.json。
   * 面板「拉取模型」按钮用：把上游真实模型名列出来点选，避免手打模型名填错。
   *
   * 入参两种形态（可叠加，优先级：显式入参 > 已保存渠道的原值）：
   *   { name }                                  已保存的渠道：用它的端点/密钥/请求头/代理
   *   { preset | baseUrl, protocol, apiKey(s) } 还没保存的表单值（添加渠道时表单还没落盘）
   * 编辑已保存渠道时若表单里填了新 key，会覆盖旧 key —— 改完还没保存也能先试拉一次。
   *
   * @returns {Promise<{ok:boolean, models:string[], count:number, url:string, error?:string}>}
   */
  async fetchUpstreamModels(input = {}) {
    const name = String(input.name ?? '').trim();
    const saved = name ? this.channels.find((c) => c.name === name) : null;
    if (name && !saved) throw new Error(`渠道 ${name} 不存在`);

    // 表单里新填的 key 优先；没填就沿用已保存渠道的 key。
    // 面板从不回显明文，所以"空"只能解释成"沿用"，绝不能当成"没密钥"。
    const rawKeys = input.apiKeys !== undefined ? input.apiKeys : input.apiKey;
    const typed = (Array.isArray(rawKeys) ? rawKeys : rawKeys === undefined || rawKeys === null ? [] : [rawKeys])
      .map((k) => String(k ?? '').trim())
      .filter(Boolean);

    const raw = {
      name: name || '(未保存)',
      apiKeys: typed.length ? typed : saved ? saved.apiKeys : [],
      headers: { ...(saved?.headers || {}), ...(input.headers || {}) },
    };
    // 端点三选一：显式 preset > 显式 baseUrl > 已保存渠道的原值
    if (input.preset) {
      raw.preset = String(input.preset);
    } else if (input.baseUrl) {
      raw.baseUrl = String(input.baseUrl);
      raw.protocol = input.protocol || saved?.protocol || 'openai';
    } else if (saved) {
      // saved.baseUrl 已是归一化值；再过一次 normalizeBaseUrl 是幂等的（不会补出 /v1/v1）
      raw.baseUrl = saved.baseUrl;
      raw.protocol = saved.protocol;
    } else {
      throw new Error('未选预设时必须填 baseUrl');
    }
    // 端点细节（自定义路径 / 版本头 / 免费档伪装 / 只认流式）沿用已保存渠道
    if (saved) {
      raw.modelsPath = saved.modelsPath;
      raw.chatPath = saved.chatPath;
      raw.anthropicVersion = saved.anthropicVersion;
      raw.opencodeFree = saved.opencodeFree;
      raw.forceStream = saved.forceStream;
      raw.refreshToken = saved.refreshToken;
    }
    if (input.modelsPath) raw.modelsPath = String(input.modelsPath);
    if (input.proxy !== undefined) raw.proxy = input.proxy;
    else if (saved) raw.proxy = saved.proxy;

    // 复用 Channel 构造器做预检：非法 preset / 缺 baseUrl / 坏协议在这里就抛错，
    // 不用等到发请求才报一个看不懂的错。
    const ch = new Channel(raw, this.routing, this.presets);
    const { url, headers } = ch.adapter.buildModelsRequest(ch);
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const detail = truncate(String(body).replace(/\s+/g, ' ').trim(), 200);
        // 没有可用 key（空 / 未展开的 ${ENV} 占位）时补一句提示，别让用户对着 401 猜
        const hint = ch.usableKeys.length ? '' : '（该渠道当前没有可用的 API Key：填了 key 也可能是 ${ENV} 占位没展开）';
        return {
          ok: false, models: [], count: 0, url,
          error: `上游返回 HTTP ${res.status}${detail ? `：${detail}` : ''}${hint}`,
        };
      }
      const json = await res.json();
      const models = ch.adapter.parseModels(json);
      if (!models || !models.length) {
        return { ok: false, models: [], count: 0, url, error: '上游返回的模型列表为空' };
      }
      return { ok: true, models, count: models.length, url };
    } catch (err) {
      return {
        ok: false, models: [], count: 0, url,
        error: err.name === 'AbortError' ? '拉取超时（15s）' : err.message || String(err),
      };
    } finally {
      clearTimeout(timer);
    }
  ...
  addChannel(input = {}) {
    ...
    // 叠加 key：apiKeys 数组（也兼容把 apiKey 写成数组 / 多行字符串）。
    // 只有 1 个 key 时仍落成 apiKey 字符串（保持旧配置文件形态与可读性）。
    const rawKeyInput = input.apiKeys !== undefined
      ? input.apiKeys
      : Array.isArray(input.apiKey)
        ? input.apiKey
        : String(input.apiKey ?? '').split(/[\r\n,]+/);
    const keys = (Array.isArray(rawKeyInput) ? rawKeyInput : [rawKeyInput])
      .map((k) => String(k ?? '').trim())
      .filter(Boolean);
    if (keys.length === 1) entry.apiKey = keys[0];
    else if (keys.length > 1) entry.apiKeys = keys;
    // 叠 Key 内部策略（race | rotate-429）：只对多 Key 渠道落盘，单 Key 不写（保持旧配置形态）。
    // 非法值交给下面的 Channel 预检统一报错，不在这里静默吞掉。
    if (keys.length > 1 && input.stackedKeyStrategy !== undefined
      && String(input.stackedKeyStrategy).trim() !== '') {
      entry.stackedKeyStrategy = String(input.stackedKeyStrategy).trim();
    }
    ...
    if (probe.keyMissing) entry.enabled = false; // 没 key 先停用，填上 key 后热重载自动启用

    raw.channels.push(entry);
    writeFileSync(file, JSON.stringify(raw, null, 2) + '\n', 'utf8');
    this.load();
    const added = this.channels.find((c) => c.name === name);
    const strategyNote = probe.stackedKeyStrategy === 'rotate-429' ? 'Key 内部轮转 + 429 快切' : '并行竞速';
    log.ok(
      `新增渠道 ${name}${probe.keyMissing ? '（未填 key，已停用）' : probe.usableKeys.length > 1 ? `（叠加 ${probe.usableKeys.length} 个 key，${strategyNote}）` : ''}`,
    );
  ...
  /**
   * 编辑已保存的渠道：按字段窄写回 config.json 并热重载（面板"编辑"按钮用）。
   *
   * 语义约定（都很关键）：
   *   1. **只覆盖显式传进来的字段**，其余（headers / chatPath / timeoutMs / creditRemain 等）
   *      从旧条目原样保留——避免"编辑一次就把预设展开的字段或运行时字段冲掉"。
   *   2. **密钥类字段留空 = 保持不变**：apiKey / apiKeys / refreshToken 面板从不回显明文，
   *      所以空值只能解释为"不改"，绝不能当成"清空"（否则用户改个备注就会把 key 抹掉）。
   *   3. **name 可改**：改名时同步把 routes 里的旧名引用改成新名，不留悬空引用。
   *   4. **落盘前用真实 Channel 构造器预检**：坏配置直接抛错、不写进文件（与 addChannel 一致）。
   * @returns 更新后渠道的 toJSON 快照
   */
  updateChannel(name, input = {}) {
    const file = this.configPath;
    const raw = JSON.parse(readFileSync(file, 'utf8'));
    if (!Array.isArray(raw.channels)) raw.channels = [];
    const idx = raw.channels.findIndex((c) => c && c.name === name);
    if (idx < 0) throw new Error(`渠道 "${name}" 不存在`);
    const old = raw.channels[idx];
    // 复制旧条目：未编辑的字段（headers / chatPath / 运行时回写的 creditRemain…）保持原样
    const entry = { ...old };

    // ---- 渠道名（支持改名）----
    const newName = input.name !== undefined ? String(input.name).trim() : name;
    if (!newName) throw new Error('缺少渠道名 name');
    if (newName !== name && raw.channels.some((c) => c && c.name === newName)) {
      throw new Error(`渠道名 "${newName}" 已存在`);
    }
    entry.name = newName;
    ...
    if (input.preset !== undefined) {
      ...
      if (preset) {
        if (!this.presets[preset]) throw new Error(`预设 "${preset}" 不存在（--list-presets 查看）`);
        entry.preset = preset;
        // 换成预设后清掉手填端点：留着会覆盖预设的 baseUrl（applyPreset 里渠道字段优先）
        delete entry.baseUrl;
        delete entry.protocol;
      } else {
        delete entry.preset;
        // 未提供 baseUrl 时回落到旧条目：让"预设渠道转自定义"和"自定义渠道只改别的字段"
        // 都不必强制重传 baseUrl（旧条目没有 baseUrl 才真的算缺参数）。
        const baseUrl = input.baseUrl !== undefined
          ? String(input.baseUrl).trim()
          : String(entry.baseUrl || '').trim();
        if (!baseUrl) throw new Error('未选 preset 时必须填 baseUrl');
        entry.baseUrl = baseUrl;
        entry.protocol = String(input.protocol || entry.protocol || 'openai').toLowerCase();
      }
    } else if (input.baseUrl !== undefined) {
      // 带 preset 的渠道也允许单独覆盖 baseUrl
      const baseUrl = String(input.baseUrl).trim();
      if (!baseUrl) throw new Error('baseUrl 不能为空');
      entry.baseUrl = baseUrl;
      if (input.protocol !== undefined) entry.protocol = String(input.protocol || 'openai').toLowerCase();
    }

    // ---- API Key：留空 = 保持不变（见方法头注释第 2 条）----
    const rawKeyInput = input.apiKeys !== undefined
      ? input.apiKeys
      : Array.isArray(input.apiKey)
        ? input.apiKey
    ...
    if (rawKeyInput !== undefined) {
      ...
      if (keys.length) {
        ...
        if (keys.length === 1) entry.apiKey = keys[0];
        else entry.apiKeys = keys;
      }
    }
    // 叠 Key 内部策略归一：只有多 Key 才有意义，单 Key / 无 Key 一律清掉（保持旧配置形态）
    const keyCount = Array.isArray(entry.apiKeys) ? entry.apiKeys.length : (entry.apiKey ? 1 : 0);
    if (keyCount > 1 && input.stackedKeyStrategy !== undefined
      && String(input.stackedKeyStrategy).trim() !== '') {
      entry.stackedKeyStrategy = String(input.stackedKeyStrategy).trim();
    } else if (keyCount <= 1) {
      delete entry.stackedKeyStrategy;
    }

    // ---- 模型绑定（与 README 口径一致：model 与 models 同时出现时以 model 为准）----
    if (Array.isArray(input.models) && input.models.length) {
      entry.models = input.models.map((m) => String(m)).filter(Boolean);
      delete entry.model;
    }
    if (input.model !== undefined) {
      ...
      if (model) {
        entry.model = model;
        delete entry.models;
      } else if (!Array.isArray(entry.models) || !entry.models.length) {
        delete entry.model;   // 显式清空且没有白名单 = 回到自动发现
      }
    }

    // ---- 其它可编辑字段（空值 = 删除该字段，回到"跟随全局/无"）----
    if (input.effort !== undefined) {
      const effort = String(input.effort || '').trim().toLowerCase();
      if (effort) entry.effort = effort; else delete entry.effort;
    }
    if (input.maxEffort !== undefined) {
      const maxEffort = String(input.maxEffort || '').trim().toLowerCase();
      if (maxEffort) entry.maxEffort = maxEffort; else delete entry.maxEffort;
    }
    if (input.priority !== undefined && Number.isFinite(Number(input.priority))) {
      entry.priority = Number(input.priority);
    }
    ...
    if (input.proxy !== undefined) {
      ...
      if (proxy) entry.proxy = proxy; else delete entry.proxy;
    }
    // refreshToken：留空 = 保持不变（与 apiKey 同理）
    if (input.refreshToken !== undefined && String(input.refreshToken).trim() !== '') {
      entry.refreshToken = String(input.refreshToken).trim();
    }
    // headers：按 key 合并（只动显式传进来的头），空值 = 删除该头
    if (input.headers && typeof input.headers === 'object') {
      const headers = { ...(entry.headers || {}) };
      for (const [k, v] of Object.entries(input.headers)) {
        if (v === undefined || v === null || String(v) === '') delete headers[k];
        else headers[k] = v;
      }
      if (Object.keys(headers).length) entry.headers = headers; else delete entry.headers;
    }

    // ---- 启用状态 ----
    // 旧条目是否"因缺 key 而不可用"：用于判断"补上 key 后要不要自动重新启用"
    let oldKeyMissing = false;
    try { oldKeyMissing = new Channel({ ...old }, this.routing, this.presets).keyMissing; } catch { oldKeyMissing = false; }
    if (typeof input.enabled === 'boolean') entry.enabled = input.enabled;
    else if (entry.enabled === false && oldKeyMissing) entry.enabled = true; // 补上 key 了 -> 自动启用

    // ---- 预检：坏配置不落盘（与 addChannel 一致）----
    const probe = new Channel({ ...entry }, this.routing, this.presets);
    if (probe.apiKeys.length === 0) {
      entry.enabled = false;    // 真的一个 key 都没有：停用
    } else if (probe.keyMissing && typeof input.enabled !== 'boolean') {
      // 只有未展开的 ${ENV} 占位：不写死 enabled=false，否则环境变量注入后也永远起不来
      delete entry.enabled;
    }

    raw.channels[idx] = entry;
    // 改名时同步 routes 引用，避免留下指向已不存在渠道的悬空项
    if (newName !== name && raw.routes && typeof raw.routes === 'object') {
      for (const k of Object.keys(raw.routes)) {
        if (Array.isArray(raw.routes[k])) {
          raw.routes[k] = raw.routes[k].map((n) => (n === name ? newName : n));
        }
      }
    }
    writeFileSync(file, JSON.stringify(raw, null, 2) + '\n', 'utf8');
    this.load();
    const updated = this.channels.find((c) => c.name === newName);
    log.ok(`已更新渠道 ${name}${newName !== name ? ` → ${newName}` : ''}`);
