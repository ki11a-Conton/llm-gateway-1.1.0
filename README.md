...
- **规则 E**：一次请求默认最多扫描 Key 池**一整圈**；整圈都失败才把错误交回**渠道级**降级逻辑（例外的"唯一候选渠道"见规则 G）。
- **规则 F**：普通业务错误（如 400 参数错误）**不盲扫**后续 Key——整池重试无意义，直接交回渠道级。
- **规则 G（唯一候选渠道 → 整圈失败后回 K1 重扫）**：如果本次请求**没有别的候选渠道可换**（候选列表只剩这一家，交回渠道级也没人接），整圈 Key 都失败后**回到第一个 Key 再扫一圈**，而不是当场失败——429 的含义是"这个 Key 这一小段窗口的额度用完了"，转完一圈往往已经过了窗口，K1 可能又能用。
  - **回到 K1 是真的重置游标**：环形游标跑完整圈会停在"这一圈的起点"（起点是 K3 就还在 K3），所以重扫前会显式把游标置回第一个 Key。
  - **终止条件**：某个 Key 成功 / 遇到规则 F 的业务错误 / 请求被中止（客户端断开、首字节超时 `firstByteTimeoutMs`、请求级总预算）——所以不是无界死循环。
  - **有别的候选渠道时不会重扫**：立刻交回渠道级快速降级换家。否则一个"整池 429"的渠道会被拖到首字节超时，反而错过本可以成功的兜底渠道。
  - 只对"可切 Key 的错误"（规则 B）重扫；整圈都是网络错误（上游直接连不上）不重扫，交给渠道级按 `retryPerAttemptMs` 节流重试，避免对着挂掉的上游热循环。
  - 可观测：`/api/metrics` 的 `stackedKeys.stackedKeyLapRetries` 记录"回 K1 重扫"发生的圈数。
